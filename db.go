...
func (db *DB) Put(key, value []byte) error {
	...
	logRecord := &data.LogRecord{
		Key:   logRecordWithSeqNo(key, NonTxnSeqNo),
		Value: value,
		Type:  data.LogRecordNormal,
	}

	recordPos, err := db.appendLogRecordWithLock(logRecord)
	if err != nil {
		return err
	}
	// 更新内存索引
